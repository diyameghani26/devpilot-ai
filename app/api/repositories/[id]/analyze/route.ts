...
import { analyzeRepository } from "@/lib/services/repository-analysis.service";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { analysis } = await analyzeRepository(githubUrl, "main");
    ...
    return NextResponse.json({ success: true, repository, analysis });
  } catch (error: any) {
