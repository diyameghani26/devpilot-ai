...
import { ArrowLeft, FolderGit2, Info, LoaderCircle, Network, Sparkles } from "lucide-react";
...
export function RepositoryWorkspacePage({ repositoryId }: { repositoryId: string }) {
  ...
  return <main className="min-h-screen bg-background">
    ...
    <div className="mx-auto max-w-4xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      {repository ? <><Link href={`/repositories/${repositoryId}/architecture`} className="mt-6 inline-flex h-8 items-center justify-center gap-2 rounded-[calc(var(--radius)-0.15rem)] border border-border bg-secondary px-3 text-xs font-medium text-secondary-foreground shadow-sm transition-colors hover:bg-accent"><Network className="size-3.5" />Architecture Explorer</Link><RepositoryFileExplorer repositoryId={repositoryId} /></> : null}
    </div>
  </main>;
}
