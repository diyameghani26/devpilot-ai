...
function GitHubMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={className}>
      <path d="M12 2C6.477 2 2 6.586 2 12.242c0 4.522 2.865 8.36 6.839 9.716.5.095.683-.222.683-.494 0-.243-.009-.888-.014-1.744-2.782.621-3.369-1.374-3.369-1.374-.455-1.183-1.11-1.498-1.11-1.498-.908-.637.068-.624.068-.624 1.004.073 1.532 1.055 1.532 1.055.892 1.566 2.341 1.114 2.91.852.09-.664.349-1.114.635-1.37-2.22-.26-4.555-1.139-4.555-5.068 0-1.12.391-2.036 1.03-2.754-.103-.26-.446-1.306.098-2.723 0 0 .84-.277 2.75 1.052A9.336 9.336 0 0 1 12 6.46c.85.004 1.705.118 2.504.347 1.91-1.329 2.748-1.052 2.748-1.052.546 1.417.203 2.463.1 2.723.64.718 1.028 1.633 1.028 2.754 0 3.94-2.338 4.805-4.565 5.06.359.32.678.952.678 1.919 0 1.385-.012 2.503-.012 2.843 0 .275.18.595.688.493C19.14 20.598 22 16.762 22 12.242 22 6.586 17.523 2 12 2Z" />
    </svg>
  );
}
...
export function RepositoryUploadPage() {
  ...
  const beginUpload = React.useCallback(() => {
    const hasSource = Boolean(file) || Boolean(url.trim());
    if (!hasSource) {
      setError("Add a GitHub URL or select a ZIP file to continue.");
      setState("error");
      return;
    }
    ...
    const timer = window.setInterval(() => {
      setProgress((value) => {
        ...
        return Math.min(value + Math.max(5, Math.round((100 - value) / 6)), 92);
      });
    }, 220);
  }, [file, isGitHubUrl, url]);
    ...
    <main className="min-h-screen bg-background">
      <header className="border-b border-border bg-background/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
          <Link href="/dashboard" className="inline-flex items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-foreground"><ArrowLeft className="size-4" />Back to dashboard</Link>
          <div className="flex items-center gap-2 text-sm font-medium"><span className="grid size-7 place-items-center rounded-lg bg-primary text-primary-foreground"><FolderGit2 className="size-4" /></span>DevPilot AI</div>
        </div>
      </header>

      <div className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="max-w-2xl">
          <Badge variant="info"><Plus className="size-3" />New repository</Badge>
          <h1 className="mt-5 text-3xl font-semibold tracking-[-0.045em] sm:text-4xl">Bring your codebase into focus.</h1>
          <p className="mt-3 max-w-xl text-base leading-7 text-muted-foreground">Connect a GitHub repository or upload a ZIP archive. DevPilot will prepare it for analysis locally in this interface.</p>
        </motion.div>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
          <Card className="overflow-hidden"><CardHeader className="border-b border-border"><CardTitle>Add a repository</CardTitle><CardDescription>Choose one source to get started.</CardDescription></CardHeader><CardContent className="space-y-8 p-5 sm:p-7">
            <section aria-labelledby="github-url-title"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-secondary"><GitHubMark className="size-4" /></span><div><h2 id="github-url-title" className="text-sm font-medium">GitHub repository URL</h2><p className="mt-0.5 text-xs text-muted-foreground">Public repositories only in this preview.</p></div></div><div className="mt-4 flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Link2 className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={url} onChange={(event) => { setUrl(event.target.value); setFile(null); setError(""); if (state !== "uploading") setState("idle"); }} placeholder="https://github.com/owner/repository" className="pl-9" disabled={state === "uploading"} /></div><Button type="button" onClick={beginUpload} disabled={state === "uploading" || !url.trim()}><GitHubMark className="size-4" />Connect</Button></div></section>
            ...
            <section aria-labelledby="zip-upload-title"><h2 id="zip-upload-title" className="sr-only">ZIP file upload</h2><motion.div layout className={cn("relative overflow-hidden rounded-xl border border-dashed p-6 text-center transition-colors sm:p-10", isDragging ? "border-ring bg-accent/60" : "border-border bg-surface hover:border-foreground/25 hover:bg-surface-raised", state === "error" ? "border-destructive/50" : "")} onDragEnter={(event) => { event.preventDefault(); setIsDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setIsDragging(false)} onDrop={(event) => { event.preventDefault(); setIsDragging(false); selectFile(event.dataTransfer.files[0]); }}>
              <input ref={inputRef} type="file" accept=".zip,application/zip,application/x-zip-compressed" className="sr-only" onChange={(event) => selectFile(event.target.files?.[0])} />
              <AnimatePresence mode="wait">
                {state === "uploading" ? <motion.div key="uploading" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}><span className="mx-auto grid size-12 place-items-center rounded-xl bg-secondary"><LoaderCircle className="size-5 animate-spin" /></span><h3 className="mt-4 text-sm font-medium">Preparing your repository</h3><p className="mt-1 text-xs text-muted-foreground">Uploading and validating archive structure…</p><div className="mx-auto mt-5 max-w-sm"><div className="h-2 overflow-hidden rounded-full bg-muted"><motion.div className="h-full rounded-full bg-foreground" animate={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs tabular-nums text-muted-foreground">{progress}% complete</p></div></motion.div> : state === "success" ? <motion.div key="success" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}><span className="mx-auto grid size-12 place-items-center rounded-xl bg-success/15 text-success"><CheckCircle2 className="size-5" /></span><h3 className="mt-4 text-sm font-medium">Repository ready for analysis</h3><p className="mt-1 text-xs text-muted-foreground">{file?.name ?? url.replace(/^https:\/\/github\.com\//, "")} has been added to this preview.</p><Button variant="secondary" size="sm" className="mt-5" onClick={reset}>Upload another</Button></motion.div> : <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><span className="mx-auto grid size-12 place-items-center rounded-xl border border-border bg-card shadow-sm"><FileArchive className="size-5 text-muted-foreground" /></span><h3 className="mt-4 text-sm font-medium">Drop a ZIP file here</h3><p className="mt-1 text-xs text-muted-foreground">or <button type="button" className="font-medium text-foreground underline underline-offset-4" onClick={() => inputRef.current?.click()}>browse your computer</button></p><p className="mt-4 text-[11px] text-muted-foreground">ZIP archives up to 250 MB</p>{file ? <div className="mx-auto mt-5 flex max-w-sm items-center gap-3 rounded-lg border border-border bg-card px-3 py-2 text-left"><FileCode2 className="size-4 text-muted-foreground" /><span className="min-w-0 flex-1 truncate text-xs font-medium">{file.name}</span><button type="button" className="text-muted-foreground hover:text-foreground" aria-label="Remove selected file" onClick={reset}><X className="size-4" /></button></div> : null}{file ? <Button className="mt-5" size="sm" onClick={beginUpload}><Upload className="size-3.5" />Upload archive</Button> : null}</motion.div>}
              </AnimatePresence>
            </motion.div></section>
        ...
        <section className="mt-12" aria-labelledby="recent-title"><div className="flex items-center justify-between"><div><h2 id="recent-title" className="text-lg font-semibold tracking-[-0.03em]">Recently uploaded</h2><p className="mt-1 text-sm text-muted-foreground">Repositories added to your workspace.</p></div><Link href="/dashboard" className="hidden items-center gap-1 text-sm text-muted-foreground hover:text-foreground sm:inline-flex">View workspace <ChevronRight className="size-4" /></Link></div><div className="mt-5 overflow-hidden rounded-xl border border-border bg-card">{recentRepositories.length > 0 ? recentRepositories.map(({ name, source, time, status }) => <div key={name} className="flex flex-col gap-3 border-b border-border p-4 last:border-0 sm:flex-row sm:items-center sm:gap-4"><span className="grid size-9 place-items-center rounded-lg bg-secondary"><FolderGit2 className="size-4 text-muted-foreground" /></span><div className="min-w-0 flex-1"><p className="truncate font-mono text-xs font-medium">{name}</p><p className="mt-1 text-xs text-muted-foreground">{source} · {time}</p></div><Badge variant={status === "Ready" ? "success" : "neutral"}><span className="size-1.5 rounded-full bg-current" />{status}</Badge><button type="button" className="hidden text-xs font-medium text-muted-foreground hover:text-foreground sm:block">Open</button></div>) : <div className="flex flex-col items-center px-6 py-12 text-center"><span className="grid size-10 place-items-center rounded-xl bg-secondary"><FolderGit2 className="size-5 text-muted-foreground" /></span><p className="mt-4 text-sm font-medium">No repositories yet</p><p className="mt-1 text-xs text-muted-foreground">Your uploaded repositories will appear here.</p></div>}</div></section>
      </div>
    </main>
  );
}
